...
def test012():
    ...
    source = """
        func main() -> void {
            print(int2str(undeclaredVar));  // Undeclared(Identifier(), undeclaredVar)
        }
"""
def test013():
    """Test type cannot be inferred: const = void-returning function"""
...
def test015():
    ...
    source = """
        func scopeError() -> void {
            ...
            print(int2str(localVar));  // Undeclared(Identifier(), localVar)
        }
...
"""
    expected = "Type Mismatch In Statement: ForStmt(x, IntegerLiteral(42), BlockStmt([ExprStmt(FunctionCall(Identifier(print), [FunctionCall(Identifier(int2str), [Identifier(x)])]))]))"
    assert Checker(source).check_from_source() == expected
...
"""
    expected = TypeMismatchInStatement(VarDecl("a", IntType(), BinaryOp(FunctionCall(Identifier("int2str"), [IntegerLiteral(1)]), "==", FunctionCall(Identifier("float2str"), [FloatLiteral(1.0)]))))
    assert Checker(source).check_from_source() == str(expected)
...
def test_176():
    source = """
func main() -> void {
    let b: string = int2str(1);
    let c: string = float2str(1.0);
    let d: string = bool2str(true);
    let e: string = int2str();
}
"""
...
def test_177():
    source = """
func main() -> void {
    let b: string = int2str(float2str(1.0) >= "a");
    let c: string = int2str("s");
}
"""
    # expected =  TypeMismatchInExpression(FunctionCall(Identifier("str"), [StringLiteral('s')]))
    expected = TypeMismatchInExpression(BinaryOp(FunctionCall(Identifier("float2str"), [FloatLiteral(1.0)]), ">=", StringLiteral('a')))
    assert Checker(source).check_from_source() == str(expected)
...
def test_047a():
    ...
    func foo(a: int, b:int, c : float) -> void {}
    func goo(a : bool) -> int {return 1;}
    """
        expected = "Static checking passed"
        assert Checker(source).check_from_source() == expected
# def test_071a():
...
def test_188():
    ...
    """
        expected = TypeMismatchInExpression(BinaryOp(StringLiteral('s'), ">", StringLiteral('c')))
        assert Checker(source).check_from_source() == str(expected)
def test_205():
        source = """
    func main() -> void {
